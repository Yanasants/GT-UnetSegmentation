...
working_folder = './outputs/Exec_2022-09-27-11-13-38.804020/' #precisará ser alterado conforme a pasta
...
for i in range(n_fold):
    ...
    fold_name = 'fold_%i'%i
    n_fold_folder_name = working_folder + fold_name + '/'
    #filename = ['girino_4_100_%s' % fold_name.lower()] #original
    n_fold_path = working_folder.split('/')[2].lower()
    filename = ['girino_4_2_%s' %n_fold_path +'_'+ fold_name.lower()] #para teste
    ...
    with open(n_fold_folder_name + 'outputs_prod/dice_production.txt', 'w') as file:
        file.write('mean: ' + str(round(np.mean(dice_list), 4)) + '\n')
        file.write('std dev: ' + str(round(np.std(dice_list), 4)) + '\n')
        file.write('median: ' + str(round(np.median(dice_list), 4)) + '\n')
        file.write('max: ' + str(round(np.max(dice_list),4)) + '\n')
        file.write('min: ' + str(round(np.min(dice_list),4)) + '\n')
